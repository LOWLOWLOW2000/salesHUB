...
認証（優先順）:
  ...
  B. ZOOM_STORAGE_STATE または 07_automation/zoom_auth.json — Playwright storage_state
  C. ZOOM_COOKIES_FILE — Cookie JSON（Playwright形式の配列）
  D. ZOOM_EMAIL + ZOOM_PASSWORD — メール/パスワード（後方互換）

使い方:
  ../.venv/bin/python zoom_recording_downloader.py [--dry-run] [--from DATE] [--to DATE]

セッション保存（SSO等は手動ログイン後にEnter）:
  ../.venv/bin/python zoom_recording_downloader.py --save-auth
"""
...
DOWNLOAD_BASE = BASE_DIR / "../../02_data_raw/zoom_recordings"
DEFAULT_AUTH_JSON = BASE_DIR / "zoom_auth.json"
...
ZOOM_DOWNLOAD_DIR = os.getenv("ZOOM_DOWNLOAD_DIR", str(DOWNLOAD_BASE))
...
def run(playwright: Playwright, args: argparse.Namespace) -> None:
    ...
    download_dir = Path(ZOOM_DOWNLOAD_DIR).resolve()
