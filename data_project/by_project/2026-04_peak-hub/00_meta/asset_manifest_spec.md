...
`data_project/` 側から `IS_01_hangar/<pj-slug>/` 配下の資料（PDF・録音・スライド等）を参照するためのインデックスCSV。
...
```
data_project/by_project/<YYYY-MM_pj-slug>/03_data_processed/asset_manifest.csv
```

参照される実ファイルはすべて以下の配下に置く:

```
IS_01_hangar/<YYYY-MM_pj-slug>/
```
...
| `project_slug` | string | ○ | プロジェクトスラッグ（例: `2026-04_peak-hub`）。`IS_01_hangar/<slug>/` 配下を意味する |
| `relative_path` | string | ○ | `IS_01_hangar/<project_slug>/` からの相対パス（例: `zoom_recordings/PEAK-000123_20260428.mp4`） |
...
1. **資料の追加時は `IS_01_hangar/` に置く → `build_asset_manifest.py` で再生成**
...
HANGAR_ROOT = Path("IS_01_hangar")
manifest = pd.read_csv("data_project/by_project/2026-04_peak-hub/03_data_processed/asset_manifest.csv")
...
paths = [HANGAR_ROOT / r["project_slug"] / r["relative_path"] for _, r in recs.iterrows()]
```
