...
export default async function AdminPage() {
  const { session, company } = await requireGm()
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-sm text-zinc-700">部署「{company.name}」の GM 管理</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        >
          ...
          <div className="mt-2 text-sm leading-6 text-zinc-600">案件とメンバー（D/AS/IS/FS/CS）割当</div>
        </Link>
        ...
        <Link
          ...
          className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm transition hover:border-zinc-300 hover:shadow"
        >
          <div className="text-sm font-semibold">GM ダッシュボード</div>
          <div className="mt-2 text-sm leading-6 text-zinc-600">全案件の CallLog / Deal 集計</div>
        </Link>
      </div>
    </div>
  )
