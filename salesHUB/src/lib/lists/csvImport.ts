...
export const parseListCsv = (raw: string): ParsedListRow[] => {
  ...
  const companyNameIndex = getIndex(headers, ['company', 'companyname', '会社名'], 0)
  const phoneIndex = getIndex(headers, ['phone', 'tel', 'phonenumber', '電話番号'], 1)
  const addressIndex = getIndex(headers, ['address', '住所'], 2)
  const targetUrlIndex = getIndex(headers, ['url', 'website', 'targeturl', '企業url'], 3)
  const industryIndex = getIndex(headers, ['industry', 'industrytag', '業種'], 4)
