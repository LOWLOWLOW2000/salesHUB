...
const SidebarNav = ({ pathname, items, projects, profile, onNavigate }: SidebarNavProps) => {
  return (
    <div className="space-y-4">
      {profile ? (
        <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2">
          ...
          <div className="mt-0.5 truncate text-xs text-zinc-600">{profile.email}</div>
          <div className="mt-2 inline-flex rounded-md bg-zinc-900 px-2 py-1 text-[10px] font-semibold tracking-wide text-white">
            {profile.roleLabel}
          </div>
        </div>
      ) : null}
        ...
        <div className="flex items-start gap-4">
          <aside className="hidden w-64 shrink-0 lg:block">
            <div className="sticky top-20 rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm">
              <SidebarNav pathname={pathname} items={items} projects={projects} profile={profile} />
            </div>
          </aside>
