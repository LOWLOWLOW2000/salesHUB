...
PJシートをDrive/Sheets APIでスナップショット保存する場合は、追加で Google API の依存も入れてください:
...
| `src/export_pj_sheet_snapshot.py` | PJシート（スプシ）を `raw/gdrive_export/` に XLSX/CSV で保存 |
...
4. デフォルトで `tools/zoom_auth.json` に保存されます（`--save-auth-path` で変更可）
