"""
PJ_asset_Data/<project_slug>/ をスキャンして asset_manifest.csv を生成する。

使い方:
  python build_asset_manifest.py --slug 2026-04_peak-hub
  python build_asset_manifest.py --slug 2026-04_peak-hub --pj-assets /path/to/PJ_asset_Data
  python build_asset_manifest.py --all

出力:
  data_project/projects/<slug>/derived/asset_manifest.csv
...
DEFAULT_PJ_ASSETS = REPO_ROOT / "PJ_asset_Data"
PROJECTS_DIR = REPO_ROOT / "data_project" / "projects"
...
def build_manifest_for_project(
    ...
    project_assets_dir = pj_assets_root / slug
    out_dir = projects_dir / slug / "derived"
    out_path = out_dir / MANIFEST_FILENAME

    if not project_assets_dir.is_dir():
        print(f"[WARN] PJ_asset_Data 配下に slug フォルダがありません: {project_assets_dir}", file=sys.stderr)
    ...
    for absolute in iter_files(project_assets_dir):
        rel = absolute.relative_to(project_assets_dir)
...
def discover_project_slugs(projects_dir: Path) -> list[str]:
    """data_project/projects/ 配下のフォルダ名を PJ slug として列挙する"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PJ_asset_Data スキャン -> asset_manifest.csv 生成")
    parser.add_argument("--slug", help="対象PJ slug（例: 2026-04_peak-hub）")
    parser.add_argument("--all", action="store_true", help="projects/ 配下の全PJに対して実行")
    parser.add_argument(
        "--pj-assets",
        "--hangar",
        default=str(DEFAULT_PJ_ASSETS),
        dest="pj_assets",
        help="PJ_asset_Data のパス（--hangar は後方互換別名）",
    )
    ...
    pj_assets_root = Path(args.pj_assets).resolve()
    targets = (
        discover_project_slugs(PROJECTS_DIR) if args.all else [args.slug]
    )
