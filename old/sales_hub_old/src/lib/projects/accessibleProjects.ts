...
/**
 * Projects the user may open (company managers: all company projects; others: ProjectMember only)
 */
export const getAccessibleProjects = async (userId: string): Promise<AccessibleProject[]> => {
  const company = await getOrCreateDefaultCompany()
  const isManager = await hasCompanyRole(userId, company.id, 'manager')
