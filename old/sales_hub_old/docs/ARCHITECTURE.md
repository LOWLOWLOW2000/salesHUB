...
- セットアップ手順・トラブルシュート: [OAUTH_SETUP.md](./OAUTH_SETUP.md)（アプリ内の要約は `/docs/oauth-setup`）
...
- **ローカルファイル**: `DATABASE_URL="file:./dev.db"` とすると実体は `sales_hub/prisma/dev.db`（`schema.prisma` があるディレクトリ基準で解決される。`file:./prisma/dev.db` は誤りで `prisma/prisma/dev.db` になる）
- **1 URL = 1 DB ファイル**: `Project` はその SQLite の中の**テーブル行**である。UI に複数の Project（例: DIP / peak hub）が並ぶのは「DBが複数」ではなく、同一 DB 内の複数レコード。
